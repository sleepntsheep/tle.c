...
{
  const char *sql =
    "SELECT t.task_pk,t.name,EXISTS(SELECT 1 FROM submissions s WHERE s.task_pk=t.task_pk AND s.user_id=?1 AND (s.verdict='accepted' OR s.result LIKE 'accepted%')) "
    "FROM tasks t WHERE (?2='1' OR NOT EXISTS(SELECT 1 FROM submissions s WHERE s.task_pk=t.task_pk AND s.user_id=?1 AND (s.verdict='accepted' OR s.result LIKE 'accepted%'))) "
    "AND t.is_hidden=0 ORDER BY t.task_pk DESC";
  sqlite3_stmt *stmt;
  if (!show_solved) show_solved = "1";
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) return -1;
  sqlite3_bind_int(stmt, 1, user_id);
  sqlite3_bind_text(stmt, 2, show_solved, -1, SQLITE_TRANSIENT);
  size_t capacity = 16;
